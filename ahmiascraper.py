...
def Scraper():
    ...
    def findlinks(content):
        ...
        data=list(set(mineddata))
        data_Count=len(list(set(mineddata)))
        conn = mysql.connector.connect(user='root', password='', host='localhost', database='darkwebdata')
        cursor = conn.cursor()
        sql = "INSERT INTO darkweblink (keyword,data,total_link) VALUES (%s, %s, %s)"
        val = (yourquery,str(data),str(data_Count))
        
        cursor.execute(sql, val)
        conn.commit()

        # sql = "INSERT INTO trends (content, status, reach, post_url, duration, post_creator) VALUES (%s, %s, %s, %s, %s, %s)"
	    # val = (post['text'], 'pending', post['reaction_count'], post['post_url'], dt_obj, post['username'])
